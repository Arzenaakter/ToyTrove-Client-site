...
const Category = () => {
  ...
  const handleSoprtsCate = () => {
    ...
    const SportsCategory = allToys.filter((toy) =>
      categories.includes(toy.subCategory)
    );
  ...
  const handleTruck = () => {
    ...
    const truckCategory = allToys.filter((toy) =>
      categories.includes(toy.subCategory)
    );
  ...
  const handleRegularCar = () => {
    ...
    const RegularCarCategory = allToys.filter((toy) =>
      categories.includes(toy.subCategory)
    );
    ...
    <div className="my-20 px-10 lg:px-0 container  m-auto ">
      ...
      <Tabs>
        <TabList className="bg-base-200 pt-5 text-center border-b-2 mb-6 ">
          ...
          <Tab onClick={handleSoprtsCate}>
            {" "}
            <span className="text-red-600 font-bold">Sports car</span>{" "}
          </Tab>

          <Tab onClick={handleTruck}>
            <span className="text-blue-600 font-bold">Truck</span>
          </Tab>

          <Tab onClick={handleRegularCar}>
            <span className="text-red-600 font-bold">Regular car</span>{" "}
          </Tab>
        </TabList>

        <TabPanel data-aos="zoom-in">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {allToys.slice(0, 6).map((subCate) => (
              <SubCategory key={subCate._id} subCate={subCate}></SubCategory>
            ))}
          </div>
        </TabPanel>
        <TabPanel data-aos="zoom-in-up">
          <div>
            <div
              className="flex  my-5  font-bold   justify-center "
              data-aos="fade-left">
